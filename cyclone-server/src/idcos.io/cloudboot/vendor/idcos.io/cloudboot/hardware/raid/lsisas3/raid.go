...
func (worker *worker) Controllers() (ctrls []raid.Controller, err error) {
	...
	for _, id := range ids {
		ctrl, err := worker.findController(id)
		if err != nil {
			return nil, err
		}
...
func (worker *worker) InitLogicalDrives(ctrlID string) error {
	...
	// _, err := worker.Base.Exec("dd", "if", "/dev/urandom", "of", "/dev/sda", "bs", "512", "count", "2")
...
func (worker *worker) SetJBODs(ctrlID string, drives []string) (err error) {
	// 参考: http://support.huawei.com/enterprise/docinforeader!loadDocument1.action?contentId=DOC1000004345&partNo=10102#it_server_sas2308_200015_2
	_, err = worker.ExecByShell(tool, ctrlID, "delete", "noprompt")
	worker.Sleep()
	return err
}
