...
const (
	// name RAID处理器名称
	name = raid.HPSmartArray
	// tool 硬件配置工具
	tool = "/usr/sbin/hpssacli"
)
...
func (worker *worker) Clear(ctrlID string) (err error) {
	if !worker.logicalDrivesExist(ctrlID) {
		if log := worker.Base.GetLog(); log != nil {
			log.Warnf("Skip deleting target devices")
		}
		return nil
	}
	// hpssacli ctrl slot=%s delete forced override
...
func (worker *worker) InitLogicalDrives(ctrlID string) error {
	...
	// _, err := worker.Base.ExecByShell("dd", "if=/dev/urandom", "of=/dev/sda", "bs=512", "count=2")
...
func (worker *worker) PhysicalDrives(ctrlID string) (pds []raid.PhysicalDrive, err error) {
	...
	for i := range items {
		pds = append(pds, raid.PhysicalDrive{
			...
			MediaType:    items[i].InterfaceType,
			ControllerID: ctrlID,
		})
	}
	return pds, nil
...
func (worker *worker) CreateArray(ctrlID string, level raid.Level, drives []string) (err error) {
	...
	// 将当前RAID卡设置为RAID模式
	_ = worker.setManagingMode(ctrlID, raidmode)

	worker.Sleep()
	// hpssacli ctrl slot=%s create type=ld drives=%s raid=%s
	_, err = worker.ExecByShell(tool,
		"ctrl",
		fmt.Sprintf("slot=%s", ctrlID),
		"create",
		"type=ld",
		fmt.Sprintf("drives=%s", strings.Join(drives, ",")),
		fmt.Sprintf("raid=%s", worker.strLevel(level)),
	)
	worker.Sleep()
	return err
}
...
func (worker *worker) SetGlobalHotspares(ctrlID string, drives []string) (err error) {
	...
	return worker.hotspare(fmt.Sprintf("ctrl slot=%s array all", ctrlID), drives) // 对指定slot下所有阵列做热备
}

// SetJBODs 将指定RAID卡控制器设置为直通模式，或者将RAID卡控制器下部分物理驱动器设置为直通模式。
// ctrlID 表示RAID卡控制器ID，通过Convert2ControllerID方法可获取到第N块RAID卡的控制器ID。
// drives 物理驱动器列表。HP SmartArray系列RAID卡暂不支持将部分物理驱动器设置为直通模式，因此该参数不会产生实际效果。
func (worker *worker) SetJBODs(ctrlID string, drives []string) (err error) {
	// hpssacli ctrl slot=x modify hbamode=on forced
	_, err = worker.ExecByShell(tool, "ctrl", fmt.Sprintf("slot=%s", ctrlID), "modify", "hbamode=on", "forced")
	worker.Sleep()
	return err
}
