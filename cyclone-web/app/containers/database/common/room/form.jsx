...
class MyForm extends React.Component {
  ...
  componentDidMount() {
    ...
    if (type.indexOf('detail') !== -1 || type === '_update') {
      get(`/api/cloudboot/v1/server-rooms/${id}`).then(res => {
        ...
        this.setState({
          initialValue: {
            ...
            first_server_room: res.content.first_server_room.id
          }
        });
      });
    }
    ...
    return (
      <div>
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem {...formItemLayout} label='所属一级机房' >
            {getFieldDecorator('first_server_room', {
              ...
              <Select disabled={!showSubmit} >
                {
                  (idc_data.first_server_room || []).map(item => <Option value={item.id} key={item.id}>{item.name}</Option>)
                }
              </Select>
            )}
          </FormItem>
          ...
          <FormItem {...formItemLayout} label='机房负责人' >
            {getFieldDecorator('server_room_manager', {
              initialValue: initialValue.server_room_manager,
              rules: [
                {
                  required: true,
                  message: '请输入机房负责人'
          ...
          <FormItem {...formItemLayout} label='网络资产负责人' >
            {getFieldDecorator('network_asset_manager', {
              initialValue: initialValue.network_asset_manager,
              rules: [
                {
                  required: true,
                  message: '请输入网络资产负责人'
          ...
          <FormItem {...formItemLayout} label='7*24小时保障电话' >
            {getFieldDecorator('support_phone_number', {
              initialValue: initialValue.support_phone_number,
              rules: [
                {
                  required: true,
                  message: '请输入7*24小时保障电话'
                }
              ]
            })(<Input disabled={!showSubmit} />)}
          </FormItem>
